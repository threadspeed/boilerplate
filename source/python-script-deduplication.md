---
title: python script deduplication (snippet)
date: 2020-03-03
tags: ["python"]
---
Python script 'deduplication'
...
def gen(term, sz):
    ...
    if term not in gram:
        if sz == 1:
            sign = get_signature(term, points)
            if sign not in foundcache[ts]:
                foundcache[ts].add(sign)
