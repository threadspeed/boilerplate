---
title: python script blt (snippet)
date: 2020-03-03
tags: ["python"]
---
Python script 'blt'
...
def show():
  ...
  gist = sg.create(name=str(datetime.now()) + " " + current_script_path.name,
  ...
  for fignum in plt.get_fignums():
    ...
    with tempfile.NamedTemporaryFile(suffix=".png") as pngfile:
      ...
      pngfile.seek(0)
      resp = requests.post("https://img.onl/api/upload.php", files={"imgFile": pngfile}).json()
    assert resp["success"]
