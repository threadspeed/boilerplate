---
title: python script n ddakji (snippet)
date: 2020-03-03
tags: ["python"]
---
Python script 'n ddakji'
...
class Ddakji:
    ...
    def lesson(self, eat):
        ...
        if now_level < self.level:
            self.dance = self.dance + (self.level-1) * 700
            self.vocal = self.vocal + (self.level-1) * 700
            self.visual = self.visual + (self.level-1) * 700
...
iljjin = Ssrare('Sibuya Rin', 'Cool', 5000, 6000, 7000)
smr = Rare('Uzuki', 'Cute', 1000, 1000, 1000)
captain = Rare('Honda the CAPTAIN mio', 'Passion', 1000, 1000, 1000)

iljjin.lesson([smr, captain])
print(iljjin.who)
